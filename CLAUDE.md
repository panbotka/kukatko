...
- `internal/ctl` — **client** of the own API for `kukatko ctl`: contexts (kubectl-style), Bearer token, table/JSON/llm output, streamed renditions, the photo-edit body, and the whole curation write surface (faces/subjects/clusters, albums, labels, stacks, image edits, saved searches, duplicates, comments)
