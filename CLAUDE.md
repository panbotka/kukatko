...
- `internal/family` — the genealogy over subjects: the family is the node (a couple, a lone parent or a bare sibling group, plus their children), everything else is derived; byte-ordered pairs, one family per child, cycle check, one read walk: the whole reachable family as a capped network (the `family:` filter keeps its own descendant walk on purpose)
...
- `internal/personme` — resolves `person:me`/`family:me` to the caller's linked subject; pure, so `internal/query` stays caller-blind
...
- `internal/storekeys` — what lives in the object store and which layout owns it: one `Kind` per prefix, so a new one cannot be forgotten by backup, migration or wipe
