...
// checker, whether a passkey ceremony can be run at all, and whether uploaded
// videos are encoded for streaming — plus the build metadata of this binary, the
// one source of truth for the version the UI prints, read from the server that
// runs it.
func buildCapabilitiesAPI(
	cfg *config.Config, checker *reachability.Checker, authAPI *auth.API,
) *capabilitiesapi.API {
	return capabilitiesapi.NewAPI(capabilitiesapi.Config{
		Embeddings:     checker,
		Passkeys:       authAPI.PasskeysEnabled(),
		VideoStreaming: cfg.Video.HLS.Enabled,
		Build:          version.Get(),
