...
tagged here really her? — with **Ano · Ne · Nevím** (yes · no · don't know) underneath and the
arrow keys doing the work; on a phone the three answers are a swipe. A question about a person
shows the face it is asking about cut out of the photograph, next to the picture the library has
of that person — a face twenty pixels across in a group photograph is otherwise not a question
anybody can answer — and tapping it enlarges the face without leaving the game. Out of the box
the game is about people — which kinds it asks, and in what proportion, is a config setting
rather than an accident of how much the machine happened to guess at. Questions come in rounds
of ten, deliberately mixed by person, kind, difficulty and era, and never more than two in a row
about the same person, so a session never feels like an interrogation. "Don't know" is listened
to: say it three times about one person and the game stops asking *you* about them for a while,
...
invalidates anything on its own. Stepping out to look at a photo properly (the corner link, or
`o`) opens it right there in the app — installed on a phone too, where there are no tabs — with a
„Zpět k třídění" button at the top, and coming back picks the round up where you left it — same
question, same score — rather than starting over. A leaderboard counts who has answered the most and how many days in a row, because a
...
"Datum: bez data" gathers everything left undated. The maintenance page can do the same for the dates
nobody typed: it finds the photos dated to a year no photograph can have been taken in — a Facebook
download whose asset id was read as a date lands in the year 9009, sorts ahead of the whole library and
stretches the year axis to meet it — and clears those dates on request, one by one into the same
undated pile, never inventing a replacement · placing a whole selection on the map at once, with
...
- The listing marks the tasks **somebody else has replied to since the state last moved**, shows on
