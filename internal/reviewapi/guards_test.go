...
// TestGuards_curatorPlaysTheGame proves the queue and the answer hang on
// RequireCurator: a curator and an editor pass their guard, a viewer is refused.
func TestGuards_curatorPlaysTheGame(t *testing.T) {
	t.Parallel()
	routes := []guardedRoute{
		{http.MethodGet, "/review/queue", true},
		{http.MethodPost, "/review/answer", true},
	}
