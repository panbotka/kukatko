...
const (
	// notificationUIDPrefix marks UIDs that identify notifications rows.
	notificationUIDPrefix = "nt"
...
// newNotificationUID returns a fresh UID for a notification row, of the form "nt"
// followed by uidSuffixLen random base32 characters. It returns a wrapped error
// only if the system random source fails.
func newNotificationUID() (string, error) {
	return newUID(notificationUIDPrefix)
}
...
func newUID(prefix string) (string, error) {
	if len(prefix)+uidSuffixLen > uidMaxLen {
		panic(fmt.Sprintf("notification: uid prefix %q too long for VARCHAR(%d)", prefix, uidMaxLen))
	}
...
func randomString(n int) (string, error) {
	...
	if _, err := rand.Read(buf); err != nil {
		return "", fmt.Errorf("notification: reading random bytes: %w", err)
	}
