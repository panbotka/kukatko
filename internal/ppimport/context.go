...
//
// It reads both relations off the detail its caller already holds (see
// importPhotoDetail), because the listing payload carries neither. That detail
// costs one request per photo, which is the right trade for a slice (17 photos, 17
// requests) and exactly why a full run does not fetch one per listed photo: it maps
// the same structure by walking the album and label catalogues, where a detail per
// photo would turn one listing into 20k requests. Hence the no-op when
// state.photoCtx is nil, which is how a full run is told apart from a scoped one.
//
...
func (s *Service) mapPhotoContext(
	ctx context.Context, photoUID string, detail photoprism.PhotoDetail, state *runState,
) {
	if state.photoCtx == nil {
		return
	}
	for i := range detail.Albums {
		s.attachPhotoToAlbum(ctx, photoUID, detail.Albums[i], state.photoCtx)
	}
	for i := range detail.Labels {
		s.attachPhotoLabel(ctx, photoUID, detail.Labels[i], state.photoCtx)
	}
}
