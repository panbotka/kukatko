...
const (
	// albumUIDPrefix marks UIDs that identify album rows.
	albumUIDPrefix = "al"
	// labelUIDPrefix marks UIDs that identify label rows.
	labelUIDPrefix = "lb"
...
// newAlbumUID returns a fresh UID for an album row, of the form "al" followed by
// uidSuffixLen random base32 characters. It returns a wrapped error only if the
// system random source fails.
func newAlbumUID() (string, error) {
	return newUID(albumUIDPrefix)
}

// newLabelUID returns a fresh UID for a label row, of the form "lb" followed by
// uidSuffixLen random base32 characters. It returns a wrapped error only if the
// system random source fails.
func newLabelUID() (string, error) {
	return newUID(labelUIDPrefix)
}
...
func newUID(prefix string) (string, error) {
	if len(prefix)+uidSuffixLen > uidMaxLen {
		panic(fmt.Sprintf("organize: uid prefix %q too long for VARCHAR(%d)", prefix, uidMaxLen))
	}
...
func randomString(n int) (string, error) {
	...
	if _, err := rand.Read(buf); err != nil {
		return "", fmt.Errorf("organize: reading random bytes: %w", err)
	}
