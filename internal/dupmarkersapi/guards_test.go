...
// TestGuards_curatorRepairsMarkers proves both repairs hang on RequireCurator: a
// curator and an editor pass their guard, a viewer is refused. The package has no
// route left on RequireWrite.
func TestGuards_curatorRepairsMarkers(t *testing.T) {
	t.Parallel()
	routes := []guardedRoute{
		{http.MethodPost, "/duplicate-markers/keep", true},
		{http.MethodPost, "/duplicate-markers/invalid", true},
	}
	for _, role := range []auth.Role{auth.RoleViewer, auth.RoleCurator, auth.RoleEditor} {
		router := chi.NewRouter()
		router.Route("/api/v1", dupmarkersapi.NewAPI(dupmarkersapi.Config{
