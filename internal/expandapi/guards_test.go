...
// TestGuards_curatorExpandsCollections proves both "photos similar to this
// collection" searches hang on RequireCurator: they lead into the album and label
// curation a curator owns. A curator and an editor pass, a viewer is refused.
func TestGuards_curatorExpandsCollections(t *testing.T) {
	t.Parallel()
	routes := []guardedRoute{
		{http.MethodGet, "/albums/al1/similar", true},
		{http.MethodGet, "/labels/lb1/similar", true},
	}
	for _, role := range []auth.Role{auth.RoleViewer, auth.RoleCurator, auth.RoleEditor} {
		router := chi.NewRouter()
		router.Route("/api/v1", expandapi.NewAPI(expandapi.Config{
