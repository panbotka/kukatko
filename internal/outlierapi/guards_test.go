...
// TestGuards_curatorReachesTheOutliers proves the outlier listing hangs on
// RequireCurator: a curator and an editor pass its guard, a viewer is refused.
func TestGuards_curatorReachesTheOutliers(t *testing.T) {
	t.Parallel()
	routes := []guardedRoute{
		{http.MethodGet, "/subjects/su1/outliers", true},
	}
	for _, role := range []auth.Role{auth.RoleViewer, auth.RoleCurator, auth.RoleEditor} {
		router := chi.NewRouter()
		router.Route("/api/v1", outlierapi.NewAPI(outlierapi.Config{
