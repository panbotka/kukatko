 ...
 *
 * Changing the query text or the `mode` resets and reloads from the first page;
 * the result also surfaces the effective `mode` and the `degraded` flag the
 * server sets when a semantic/hybrid search fell back to full-text.
 *
 * `params` should be memoised by the caller so its identity changes only when
 * the query actually changes.
 */
export function usePhotoSearch(params: PhotoListParams, mode: SearchMode): UsePhotoSearchResult {
  const fetcher = useCallback(
    (p: PhotoListParams, signal: AbortSignal) => searchPhotos(p, mode, signal),
    [mode],
  )
  const enabled = (params.q ?? '').trim() !== ''
  return usePaginatedPhotos(params, fetcher, { enabled, key: mode })
}
